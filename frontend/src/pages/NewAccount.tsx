...
export default function NewAccount() {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 space-y-4 text-black dark:text-white">
      ...
      <select className="block border p-2 rounded w-full dark:bg-gray-700" value={form.providerId}
      ...
      <input className="block border p-2 rounded w-full dark:bg-gray-700" placeholder={t("accounts.description")}
        value={form.userDescription} onChange={e => setForm({ ...form, userDescription: e.target.value })} />

      <input className="block border p-2 rounded w-full dark:bg-gray-700" placeholder={t("accounts.username")}
        value={form.user} onChange={e => setForm({ ...form, user: e.target.value })} />

      <input className="block border p-2 rounded w-full dark:bg-gray-700" placeholder={t("accounts.password")}
        value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />

      <input className="block border p-2 rounded w-full dark:bg-gray-700" type="date"
