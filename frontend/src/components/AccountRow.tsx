  ...
  return (
    <tr className="block md:table-row border-b md:border-none mb-4 md:mb-0">
      ...
      <td className="block md:table-cell px-4 py-2">
        ...
        <button onClick={() => navigator.clipboard.writeText(domainUrl)}><Copy className="inline w-4 h-4 ml-1" /></button>
      </td>
      <td className="block md:table-cell px-4 py-2">
        <span className="font-semibold md:hidden">User: </span>
        {account.user}
        <button onClick={() => navigator.clipboard.writeText(account.user)}><Copy className="inline w-4 h-4 ml-1" /></button>
      </td>
      <td className="block md:table-cell px-4 py-2">
      ...
      <td className="block md:table-cell px-4 py-2 flex gap-2">
        <Link to={`/accounts/edit/${account.id}`}><Pencil className="w-4 h-4" /></Link>
        <button onClick={() => onDelete(account.id)}><Trash2 className="w-4 h-4 text-red-600" /></button>
      </td>
    </tr>
  );
}
